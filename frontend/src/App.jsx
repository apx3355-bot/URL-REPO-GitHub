...
function AnnouncementTools({ onSaved }) { const [form, setForm] = useState({ title: '', content: '', duration: '7', poster: null }); const save = async (e) => { e.preventDefault(); if (!form.poster) return; const data = new FormData(); data.append('title', form.title); data.append('content', form.content); data.append('duration', form.duration); data.append('poster', form.poster); await api('announcements', { method: 'POST', body: data }); setForm({ title: '', content: '', duration: '7', poster: null }); onSaved(); }; return <section className="tool-panel"><p className="eyebrow">INFORMASI KELAS</p><h2>Pengumuman baru</h2><form className="tool-form" onSubmit={save}><input placeholder="Judul" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} required /><textarea placeholder="Isi pengumuman" value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} required /><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(e) => setForm({ ...form, poster: e.target.files[0] || null })} /><select value={form.duration} onChange={(e) => setForm({ ...form, duration: e.target.value })}><option value="1">1 hari</option><option value="7">7 hari</option><option value="30">30 hari</option><option value="permanent">Permanen</option></select><button className="button primary">Terbitkan</button></form></section>; }

function GalleryTools({ onSaved }) { const [form, setForm] = useState({ title: '', description: '', foto: null }); const save = async (e) => { e.preventDefault(); if (!form.foto) return; const data = new FormData(); data.append('title', form.title); data.append('description', form.description); data.append('foto', form.foto); await api('gallery', { method: 'POST', body: data }); setForm({ title: '', description: '', foto: null }); onSaved(); }; return <section className="tool-panel"><p className="eyebrow">KENANGAN KELAS</p><h2>Tambah foto</h2><form className="tool-form" onSubmit={save}><input placeholder="Judul foto" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} required /><textarea placeholder="Keterangan" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(e) => setForm({ ...form, foto: e.target.files[0] || null })} required /><button className="button secondary">Simpan foto</button></form></section>; }
